...
int main() {
  ...
  std::cout << "  Sequence Length:      " << sequence_length << " tokens"
            << std::endl;
  ...
  std::cout << "[INFO] Initializing DataIngestion and parsing BPE vocabulary..."
            << std::endl;
  ...
  double init_ms = std::chrono::duration_cast<std::chrono::microseconds>(
                       init_end - init_start)
                       .count() /
                   1000.0;
  std::cout << "[INFO] Pipeline initialization completed in " << init_ms
            << " ms" << std::endl
            << std::endl;
  ...
  std::cout << "==============================================================="
               "================="
            << std::endl;
  std::cout << std::left << std::setw(12) << "Batch #" << std::setw(15)
            << "Batch Size" << std::setw(18) << "Load Latency" << std::setw(20)
            << "Token Value Range"
  ...
  while (true) {
    ...
    double batch_loop_ms =
        std::chrono::duration_cast<std::chrono::microseconds>(batch_loop_end -
                                                              batch_loop_start)
            .count() /
        1000.0;
    ...
    // Print first 5 batches and then every 200,000 batches to avoid excessive
    // spam
    ...
    if (print_detail) {
      ...
      std::string token_range = "[" + std::to_string(min_token) + ", " +
                                std::to_string(max_token) + "]";
      ...
      std::cout << std::left << std::setw(12)
                << ("B-" + std::to_string(batch_num)) << std::setw(15)
                << (std::to_string(batch.size()) + " seq") << std::setw(18)
                << (std::to_string(batch_loop_ms) + " ms") << std::setw(20)
                << token_range << token_sample << std::endl;
    }
  }
  ...
  std::cout << "==============================================================="
               "================="
            << std::endl;
  std::cout << "SECTION 3: PIPELINE THROUGHPUT & PERFORMANCE SUMMARY"
            << std::endl;
  std::cout << "==============================================================="
               "================="
            << std::endl;
  ...
  if (total_sequences > 0) {
    ...
    std::cout << "  Total Sequences Streamed:  " << total_sequences
              << std::endl;
    std::cout << "  Total Tokens Processed:    " << total_tokens << std::endl;
    std::cout << "  Average Sequences / Batch: " << avg_sequences_per_batch
              << std::endl;
    std::cout << "  Average Tokens / Batch:    " << avg_tokens_per_batch
              << std::endl;
    std::cout << "  Average Tokens / Sequence: " << avg_tokens_per_sequence
              << std::endl;
    ...
    std::cout << "  Pipeline Throughput:       " << std::fixed
              << std::setprecision(0) << throughput << " tokens/second"
              << std::endl;
  } else {
    std::cout << "  [WARNING] No data was fetched. Check shard paths:"
              << std::endl;
