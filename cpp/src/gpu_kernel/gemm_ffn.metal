...
//    - shared_A: 64 x 32 floats (8 KB)
//    - shared_B_gate: 32 x 64 floats (8 KB)
//    - shared_B_up: 32 x 64 floats (8 KB)
//    Total shared memory footprint = 24 KB, which fits within the 32 KB limit of M5 GPU.
// ==============================================================================
    ...
    uint  lane_id [[thread_index_in_simdgroup]])
{
  ...
  // Arrange the 4 SIMDgroups in a 2x2 grid to compute the 64x64 tile
  uint sg_row = simd_id / 2; // 0 or 1
  uint sg_col = simd_id % 2; // 0 or 1

  // 2. Initialize Registers to Accumulate Outputs
  simdgroup_matrix<float, 8, 8> accum_gate[4][4];
  simdgroup_matrix<float, 8, 8> accum_up[4][4];
  ...
  uint tile_row_start = tg_id.y * 64;
  uint tile_col_start = tg_id.x * 64;
  ...
  for (uint k_offset = 0; k_offset < K; k_offset += 32) {
    ...
    // COLLABORATIVE LOAD: Load 32x64 slice of B_gate into shared_B_gate
    for (uint i = 0; i < 16; ++i) {
    ...
    // COLLABORATIVE LOAD: Load 32x64 slice of B_up into shared_B_up
    for (uint i = 0; i < 16; ++i) {
      uint load_idx = thread_linear_id + i * 128;
      uint load_row = load_idx / 64;
      uint load_col = load_idx % 64;
    ...
    for (uint k_step = 0; k_step < 32; k_step += 8) {
      for (int c = 0; c < 4; ++c) {
        ...
        simdgroup_load(tile_B_gate, &shared_B_gate[k_step][sg_col * 32 + c * 8], 64, ulong2(0));
        simdgroup_load(tile_B_up, &shared_B_up[k_step][sg_col * 32 + c * 8], 64, ulong2(0));

        for (int r = 0; r < 4; ++r) {
          simdgroup_matrix<float, 8, 8> tile_A;
          simdgroup_load(tile_A, &shared_A[sg_row * 32 + r * 8][k_step], 32, ulong2(0));
  ...
  for (int r = 0; r < 4; ++r) {
    for (int c = 0; c < 4; ++c) {
      ...
      uint global_row = tile_row_start + sg_row * 32 + r * 8;
      uint global_col = tile_col_start + sg_col * 32 + c * 8;
